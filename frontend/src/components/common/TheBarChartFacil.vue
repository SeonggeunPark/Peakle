  ...
  async mounted() {
    ...
    try {
      ...
      this.chartData = {
        ...
        datasets: [{ data: this.facilData.map((item) => item[1]) }], // 데이터 값
      };
      // 최대값 설정 -> data 중 최대값을 차트 최대 스케일로 설정
      // facilData에서 최대값 찾기
      const maxDataValue = Math.max(...this.facilData.map((item) => item[1]));
      this.chartOptions.scales.x.max = maxDataValue;

      // this.chartOptions.scales.x.max = Math.ceil(this.facilData[0][1]/10) *10;
    } catch (error) {
